...
def test_create(mocker):
    ...
    data = {
        "mapRoles": yaml.dump(rename_arn_keys([DATA_DEFAULT, DATA_CREATE]), default_flow_style=False)
    }
...
def test_delete(mocker):
    ...
    config_map, _ = aws_auth.write_config_map.call_args
    assert isinstance(config_map[0], kubernetes.client.V1ConfigMap)
    data = {"mapRoles": yaml.dump(rename_arn_keys([DATA_DEFAULT]), default_flow_style=False)}
    assert config_map[0].data == data
...
def test_update(mocker):
    ...
    config_map, _ = aws_auth.write_config_map.call_args
    assert isinstance(config_map[0], kubernetes.client.V1ConfigMap)
    data = {"mapRoles": yaml.dump(rename_arn_keys([DATA_UPDATE]), default_flow_style=False)}
    assert config_map[0].data == data
